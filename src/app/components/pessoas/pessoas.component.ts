...
export class PessoasComponent implements OnInit {
  formulario: any;
  tituloFormulario: string | any;
  pessoas: Pessoa[] | undefined;
  nomePessoa: string | any;
  pessoaId: Pessoa["pessoaId"] | any;
  ...
  EnviarFormulario(): void {
    ...
    if (this.pessoaId > 0) {
      this.pessoasService.AtualizarPessoa(pessoa).subscribe((resultado) => {
        ...
        this.pessoasService.PegarTodos().subscribe((registros: any) => {
  ...
  ExibirConfirmacaoExclusao(pessoaId: Pessoa["pessoaId"], nomePessoa: Pessoa["nome"], conteudoModal: TemplateRef<any>): void {
